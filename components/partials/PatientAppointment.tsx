import React from "react";
import PatientAppointmentCard, {
  PatientAppointmentCardProps,
} from "./PatientAppointmentCard";

const dummyAppointments: PatientAppointmentCardProps[] = [
  {
    name: "Rico Christian Ferry",
    status: "Today, 10.00 - 10.45",
    type: "upcoming",
    hasAiInsights: false,
  },
  {
    name: "Rico Christian Ferry",
    status: "Today, 10.00 - 10.45",
    type: "upcoming",
    hasAiInsights: true,
  },
  {
    name: "Rico Christian Ferry",
    status: "Today, 10.00 - 10.45",
    type: "upcoming_urgent",
    duration: "12 Mins",
    hasAiInsights: false,
  },
  {
    name: "Rico Christian Ferry",
    status: "Today, 10.00 - 10.45",
    type: "upcoming_urgent",
    duration: "12 Mins",
    hasAiInsights: true,
  },
  {
    name: "Rico Christian Ferry",
    status: "Patient in Waiting Room",
    type: "waiting",
    duration: "4 Mins",
    isHighlighted: true,
    hasAiInsights: false,
  },
  {
    name: "Rico Christian Ferry",
    status: "Patient in Waiting Room",
    type: "waiting",
    duration: "4 Mins Left",
    isHighlighted: true,
    hasAiInsights: true,
  },
  {
    name: "Rico Christian Ferry",
    status: "On Going Sessions",
    type: "ongoing",
    duration: "12:02",
    hasAiInsights: false,
  },
  {
    name: "Rico Christian Ferry",
    status: "On Going Sessions",
    type: "ongoing",
    duration: "12:02",
    hasAiInsights: true,
  },
  {
    name: "Rico Christian Ferry",
    status: "Meetings Ended",
    type: "ended",
    duration: "41:00",
    hasAiInsights: false,
  },
  {
    name: "Rico Christian Ferry",
    status: "Meetings Ended",
    type: "ended",
    duration: "41:00",
    hasAiInsights: true,
  },
  {
    name: "Mahendra Arya",
    status: "Yesterday",
    type: "past",
    duration: "39:12 Mins",
    hasAiInsights: false,
  },
];

export default function PatientAppointment() {
  return (
    <div className="bg-surface-background border border-border-default rounded-xl p-6 flex flex-col gap-6">
      {/* Header */}
      <div className="flex justify-between items-center">
        <div className="flex gap-3 items-center">
          <div className="text-icon-action">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="size-8"
              viewBox="0 0 24 24"
            >
              <path
                fill="currentColor"
                d="M1 18q-.425 0-.712-.288T0 17v-.575q0-1.075 1.1-1.75T4 14q.325 0 .625.013t.575.062q-.35.525-.525 1.1t-.175 1.2V18zm6 0q-.425 0-.712-.288T6 17v-.625q0-.8.438-1.463t1.237-1.162T9.588 13T12 12.75q1.325 0 2.438.25t1.912.75t1.225 1.163t.425 1.462V17q0 .425-.287.713T17 18zm12.5 0v-1.625q0-.65-.162-1.225t-.488-1.075q.275-.05.563-.062T20 14q1.8 0 2.9.663t1.1 1.762V17q0 .425-.288.713T23 18zM8.125 16H15.9q-.25-.5-1.388-.875T12 14.75t-2.512.375T8.125 16M4 13q-.825 0-1.412-.587T2 11q0-.85.588-1.425T4 9q.85 0 1.425.575T6 11q0 .825-.575 1.413T4 13m16 0q-.825 0-1.412-.587T18 11q0-.85.588-1.425T20 9q.85 0 1.425.575T22 11q0 .825-.575 1.413T20 13m-8-1q-1.25 0-2.125-.875T9 9q0-1.275.875-2.137T12 6q1.275 0 2.138.863T15 9q0 1.25-.862 2.125T12 12m0-2q.425 0 .713-.288T13 9t-.288-.712T12 8t-.712.288T11 9t.288.713T12 10m0-1"
              />
            </svg>
          </div>
          <h3 className="text-body-xl-semibold text-text-heading">
            Consultation Queue
          </h3>
        </div>
      </div>

      {/* Content List - Vertically stacked as in image */}
      <div className="flex flex-col gap-4">
        {dummyAppointments.map((item, index) => (
          <PatientAppointmentCard key={index} {...item} />
        ))}
      </div>
    </div>
  );
}
