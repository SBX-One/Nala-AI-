...
import { createArticle, getArticleCategories, getArticleTopics } from "@/app/actions/article";
...
export default function CreateArticlePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [cats, tops] = await Promise.all([getArticleCategories(), getArticleTopics()]);
      setCategories(cats);
      setTopics(tops);
    ...
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <div className="px-8 py-4 border-b border-border-default flex justify-between items-center bg-white sticky top-0 z-10">
        <div className="flex items-center gap-2 text-label-base-medium text-text-subheading">
          ...
          <span className="text-text-heading font-semibold">Create Article</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => router.back()} className="px-6 py-2 rounded-lg border border-error-500 text-error-600 font-semibold hover:bg-error-50 transition-colors">
            Delete
          </button>
          <button 
            onClick={handleSave} 
      ...
      <div className="flex-1 overflow-y-auto px-10 py-12 max-w-5xl mx-auto w-full">
        {previewMode ? (
          ...
          <>
            ...
            >
              ...
              {uploading ? (
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-label-base-semibold text-text-subheading">Uploading...</p>
                </div>
              ) : formData.imageUrl ? (
                <div className="relative w-full h-full">
                  <img src={formData.imageUrl} alt="Cover" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                    ...
                    <span className="text-white text-label-base-bold">Change Cover Image</span>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <p className="text-heading-5-bold text-text-heading">Upload Image Cover</p>
                </>
              )}
            </div>
            ...
            <div className="mt-10 flex flex-col gap-6">
              ...
              <div className="flex flex-wrap gap-3">
                  ...
                  <div className="relative group">
                    ...
                    >
                    <option value={0} disabled>Add Category</option>
                ...
                {topics.map((topic: any) => (
                  ...
                  >
                    <span className="text-lg leading-none">{formData.topicIds.includes(topic.id) ? "✓" : "+"}</span>
                    {topic.name}
                  </button>
                ))}
              </div>
            ...
            <TiptapEditor 
              content={formData.content}
              onChange={(markdown: string) => setFormData(prev => ({ ...prev, content: markdown }))}
