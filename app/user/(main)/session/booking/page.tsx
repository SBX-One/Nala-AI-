...
export default function BookingPage() {
  ...
  const [billingInfo, setBillingInfo] = useState({ cardHolder: "", cardNumber: "", expireDate: "", cvv: "" });
  const [isSuccessModalOpen, setIsSuccessModalOpen] = useState(false);
  const [lastBookingInfo, setLastBookingInfo] = useState<any>(null);

  useEffect(() => {
    async function fetchData() {
      const [psychData, expData] = await Promise.all([getPsychiatrists(), getExpertises()]);
  ...
  const getFilterSummary = () => {
    ...
    const expName = expertises.find((e) => e.id === selectedExpertise)?.name || "All Specialist";
    parts.push(expName);

    if (selectedRating) parts.push(`${selectedRating}.0+ Rating`);
    if (selectedGender) parts.push(selectedGender.charAt(0).toUpperCase() + selectedGender.slice(1));
    if (selectedExperience) parts.push(selectedExperience.charAt(0).toUpperCase() + selectedExperience.slice(1));
    if (minPrice || maxPrice) parts.push("Price Filtered");

    return parts.join(", ");
  };

  const isFiltering = selectedRating !== null || selectedGender !== null || selectedExperience !== null || minPrice !== "" || maxPrice !== "";

  const isAvailabilityFiltering = availStartDate !== null || availFromTime !== "" || availToTime !== "";

  const getAvailabilitySummary = () => {
    if (!isAvailabilityFiltering) return "";
    const parts = [];
    if (availStartDate && availEndDate) {
      const startStr = availStartDate.toLocaleDateString("id-ID", { day: "numeric", month: "short" });
      const endStr = availEndDate.toLocaleDateString("id-ID", { day: "numeric", month: "short" });
      parts.push(`${startStr} - ${endStr}`);
    } else if (availStartDate) {
      parts.push(availStartDate.toLocaleDateString("id-ID", { day: "numeric", month: "short" }));
    }
  ...
  const filteredPsychiatrists = useMemo(() => {
    return psychiatrists.filter((p) => {
      if (selectedExpertise) {
        const expName = expertises.find(e => e.id === selectedExpertise)?.name;
      ...
      if (selectedExperience) {
        if (selectedExperience === "junior" && p.experience > 2) return false;
        if (selectedExperience === "intermediate" && (p.experience <= 2 || p.experience > 5)) return false;
        if (selectedExperience === "expert" && p.experience <= 5) return false;
      }
      ...
      if (availStartDate) {
        ...
        const isAvailableInRange = p.availability.some((a: any) => {
          if (!daysInRange.includes(a.day.toLowerCase())) return false;
  ...
  }, [psychiatrists, selectedExpertise, selectedRating, selectedGender, selectedExperience, minPrice, maxPrice, expertises, availStartDate, availEndDate, availFromTime, availToTime]);
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="px-6 py-8 grid grid-cols-1 md:grid-cols-2 gap-4 bg-surface-default min-h-[600px] items-start">
        {filteredPsychiatrists.length > 0 ? (
          filteredPsychiatrists.map((p) => (
            <PsychiatristCard key={p.id} psychiatrist={p} onBookClick={handleBookClick} idr={idr} />
          ))
        ) : (
          <div className="col-span-full h-full flex flex-col items-center justify-center">
            ...
            <div className="text-center max-w-2xl px-6 grid gap-6">
              ...
              <p className="text-body-base-medium text-text-subheading leading-relaxed">
                We couldn&apos;t find a specialist matching those exact filters. Try broadening your search or resetting the filters to see more professionals ready to support you.
              </p>
            </div>
          </div>
        )}
      ...
      <FilterDrawer 
        ...
        onClear={() => {
          setTempExpertise(null); setTempRating(null); setTempGender(null);
          setTempExperience(null); setTempMinPrice(""); setTempMaxPrice("");
          setSelectedExpertise(null); setSelectedRating(null); setSelectedGender(null);
          setSelectedExperience(null); setMinPrice(""); setMaxPrice("");
        }}
      />
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-200 flex items-center justify-center px-4">
          <div className="bg-surface-background w-full max-w-md rounded-3xl p-8 flex flex-col items-center gap-6 shadow-2xl animate-in fade-in zoom-in duration-300">
            ...
            <div className="flex flex-col w-full gap-3">
              <button onClick={() => window.location.href = "/user/session/history"} className="button-primary-large w-full justify-center">View My Sessions</button>
