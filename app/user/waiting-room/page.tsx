  ...
  function DeviceTestModal({ isOpen, onClose, stream }: { isOpen: boolean, onClose: () => void, stream: MediaStream | null }) {
    ...
    const isCamDisabled = !stream?.getVideoTracks().some(t => t.readyState === "live");
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
          <div className="px-8 py-6 border-b border-border-default flex items-center justify-between">
            <h2 className="text-heading-6-bold text-text-heading">Device Settings & Test</h2>
            <button onClick={onClose} className="p-2 hover:bg-surface-default rounded-full transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" className="size-6" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M18 6L6 18M6 6l12 12"/></svg>
            </button>
          </div>
          ...
          <div className="p-8 grid gap-8 overflow-y-auto max-h-[80vh]">
            ...
            <div className="grid gap-4">
              <p className="text-label-base-bold text-text-subheading uppercase tracking-wider">Camera Test</p>
              <div className="relative aspect-video bg-black rounded-2xl overflow-hidden">
                {!isCamDisabled ? (
                  <video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover scale-x-[-1]" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-white/50 text-body-sm-medium">Camera is disabled</div>
                )}
              </div>
            </div>
            ...
            <div className="grid gap-4">
              <div className="flex items-center justify-between">
                <p className="text-label-base-bold text-text-subheading uppercase tracking-wider">Microphone Test</p>
                {isMicDisabled && <span className="text-error-default text-label-small-medium">Microphone is Muted</span>}
              </div>
              <div className="flex items-center gap-4 bg-surface-default p-4 rounded-2xl">
                <div className="size-10 rounded-full bg-white flex items-center justify-center shadow-sm">
                  <svg xmlns="http://www.w3.org/2000/svg" className="size-5" viewBox="0 0 24 24"><path fill="currentColor" d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z"/></svg>
                </div>
              ...
              >
                <div className={`size-5 rounded-full border-2 flex items-center justify-center transition-all ${isLoopback ? "border-primary-default bg-primary-default" : "border-border-default"}`}>
                  {isLoopback && <div className="size-2 rounded-full bg-white" />}
                </div>
                <div className="text-left">
                  <p className="text-body-sm-bold">Hear Myself</p>
                  <p className="text-body-caption-medium text-text-subheading">Play back your microphone audio through speakers</p>
                </div>
              </button>
              <p className="text-body-caption-medium text-text-subheading italic">Speak to see the volume indicator move. Toggle &quot;Hear Myself&quot; to listen to your mic output.</p>
            </div>

            {/* Audio Output Section */}
            <div className="grid gap-4">
              <p className="text-label-base-bold text-text-subheading uppercase tracking-wider">Speaker Test</p>
              <button 
              ...
              >
                <div className="size-10 rounded-full bg-white flex items-center justify-center shadow-sm">
                  <svg xmlns="http://www.w3.org/2000/svg" className="size-5" viewBox="0 0 24 24"><path fill="currentColor" d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/></svg>
                </div>
                <div>
                  <p className="text-body-base-bold text-text-heading">Play Test Sound</p>
                  <p className="text-body-sm-medium text-text-subheading">Click to verify your speakers are working.</p>
                </div>
              </button>
            </div>
          </div>
  ...
  function WaitingRoomContent() {
    ...
    const [timeLeft, setTimeLeft] = useState(1200); // 20:00 in seconds
        ...
        try {
          ...
          try {
            ...
            try {
              ...
              console.warn("Could not get audio either. Trying video only.", audioErr);
              setIsMuted(true);
              try {
          ...
          if (stream) {
            ...
            // Sync initial states (in case user clicked buttons while loading)
            // Use set state callbacks to get latest state if needed, but since this runs once,
            // we will just apply the state directly.
    ...
    const handleToggleMic = async () => {
      ...
      setIsMuted(nextMuted); // Update UI immediately for responsiveness
      
      if (!streamRef.current && !nextMuted) {
        try {
    ...
    const handleToggleCamera = async () => {
      ...
      setIsCameraOff(nextCameraOff); // Update UI immediately
      ...
      if (nextCameraOff) {
        ...
        if (streamRef.current && streamRef.current.getVideoTracks().some(t => t.readyState === 'live')) {
          return; // Already have active video
        }
        try {
            ...
            <div className="flex flex-col gap-6 flex-1 w-full max-w-md">
                ...
                <div className="bg-white border border-border-default rounded-3xl p-6 flex flex-col gap-6 shadow-sm">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="w-fit grid gap-3">
                      <div>
                        <p className="text-body-xl-semibold">{psychiatrist.name}</p>
              ...
              <div className="bg-white border border-border-default rounded-3xl p-8 flex flex-col items-center justify-center gap-6 shadow-sm">
                ...
                <button
                  ...
                  onClick={() =>
                    router.push(
                      `/user/active-Consultation?roomId=${roomId}`,
                    )
                  }
                >
                  {timeLeft > 0 && (
